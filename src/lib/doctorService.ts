...
export async function getAllDepartments(): Promise<string[]> {
  try {
    // Use the specializations table to get departments
    const { data: specializations, error } = await supabase
      .from('specializations')
      .select('name')
      .eq('status', 'active')
    ...
    if (error) {
      console.error('Error fetching specializations for departments:', error);
      throw new Error(`Failed to fetch departments: ${error.message}`);
    }

    // Return specialization names as departments
    return specializations?.map(s => s.name) || [];
  } catch (error) {
